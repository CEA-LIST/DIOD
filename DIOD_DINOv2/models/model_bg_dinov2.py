...
class SlotAttention(nn.Module):
    def __init__(self, num_slots, dim, eps = 1e-8, hidden_dim = 128):
        ...
        self.slots_sigma = nn.Parameter(abs(torch.randn(1, 1, dim))).to(device)
    ...
    def get_attention(self, slots, inputs):
        ...
        attn_ori_fg = 1-attn_ori[:,-1,:]
        attn_ori_fg = attn_ori_fg.unsqueeze(1)

        return slots, attn_ori, attn_ori_fg
    ...
    def forward(self, inputs, num_slots = None):
        ...
        slots, _, _ = self.get_attention(slots, inputs[:,0,:,:])
        for f in range(nf):
            cur_slots, cur_attn, attn_ori_fg = self.get_attention(slots,inputs[:,f,:,:])
            slots_out.append(cur_slots)
            attns.append(cur_attn)
            attns_ori_fg.append(attn_ori_fg)
        ...
        attns_ori_fg = torch.stack([attn for attn in attns_ori_fg])
        attns_ori_fg = attns_ori_fg.permute(1,0,2,3)
        return slots_out, attns, attns_ori_fg
...
class Encoder(nn.Module):
    def __init__(self, resolution, hid_dim, dresolution):
        super().__init__()
        self.resolution = resolution
        self.hid_dim = hid_dim
        self.dresolution = dresolution

        self.dinov2_backbone =  torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
        self.n_last_blocks = 4

        self.convGRU = ConvGRU(input_size=(self.dresolution[0],self.dresolution[1]),#(ceil(resolution[0]/4), ceil(resolution[1]/4)),
                input_dim=384*4,
                hidden_dim=hid_dim,
                kernel_size=(3,3),
                num_layers=2,
        ...
        self.encoder_pos = SoftPositionEmbed(hid_dim, (self.dresolution[0],self.dresolution[1]))

    def forward(self, x):
        bs, n_frames, N, H, W = x.shape
        x = x.view(-1, N, H,W)
        x = self.dinov2_backbone.get_intermediate_layers(x, self.n_last_blocks, return_class_token=False )
        x = torch.cat((x[0], x[1], x[2], x[3]), dim=2) # concatenated_features from last 4 layers

        # (35,70) are the spatial dims after passing bigger image into ViT-S14
        x = x.permute(0,2,1)
        x = x.view(bs*n_frames, x.shape[1], ceil(H/14), ceil(W/14))  # (35,70)
        x = F.interpolate(x, (self.dresolution[0],self.dresolution[1]))
       
        x = x.view(bs, n_frames, x.shape[1], self.dresolution[0], self.dresolution[1])  
        x,_ = self.convGRU(x)
        x = x[0]
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, hid_dim, output_channel = 3):
        ...
        self.encoder_cnn = Encoder(self.resolution, self.hid_dim, self.dresolution)
    ...
    def forward(self, image):
        ...
        # Slot Attention module.
        slots_ori, attn_masks, attn_fg = self.slot_attention(x)
        ...
        attn_fg = attn_fg.reshape(bs*n_frames,1, -1)
        attn_fg = attn_fg.view(attn_fg.shape[0],attn_fg.shape[1],self.dresolution[0], self.dresolution[1])
        ...
        return recons, attn_masks.view(bs,n_frames, self.num_slots, attn_masks.shape[2], attn_masks.shape[3]),\
            attn_fg.view(bs,n_frames, 1, attn_masks.shape[2], attn_masks.shape[3]),slots_combine
