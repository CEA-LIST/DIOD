...
from datasetKITTI_dinov2 import KITTIDataset
from datasetKITTIEval_dinov2 import KITTIDataset as KITTIDatasetEval

from  models.model_bg_dinov2 import SlotAttentionAutoEncoder
...
parser.add_argument('--exp_name', default='exp_name', type=str, help='experiment name, used for model saving/plotting ect' )
parser.add_argument('--num_workers', default=0, type=int, help='number of workers for loading data')
parser.add_argument('--seed', default=0, type=int, help='random seed')
parser.add_argument('--data_path', default = '/home/data/skara/KITTI_DOM/KITTI_DOM_train/', type = str, help = 'path of PD dataset')
...
parser.add_argument('--mean', default=[0.485, 0.456, 0.406], type = float, nargs="+", help = "normalize's mean")
parser.add_argument('--std', default=[0.229, 0.224, 0.225], type = float, nargs="+", help = "normalize's std")
# Burn_in
parser.add_argument('--burn_in_ckpt', default='DIODPD_500_dinov2.ckpt', type=str, help='ckpt from burn_in to load when starting distillation')
    ...
    for i in range(opt.batch_size):
        for j in range(5):
            ...
            for idx, s in enumerate(id_slot):
                ...
                fg_portion = torch.where(mask_gt[i,j,id_gt[idx],:,:]!=0)[0].size()[0]/(95*315)
...
def main():
    ...
    train_set = KITTIDataset(split = 'train', root = data_path, resolution = resolution, transform=train_transform, apply_img_transform=True)
    test_set = KITTIDataset(split = 'eval', root = data_path, resolution = resolution, transform = None, apply_img_transform=True)

    model = SlotAttentionAutoEncoder(resolution, opt.num_slots, opt.hid_dim, output_channel = 3)
    for param in  model.encoder_cnn.dinov2_backbone.parameters():
    ...
    for epoch in range(opt.num_epochs):
        ...
        if epoch == opt.start_teacher:
            ...
            for param in  model.module.encoder_cnn.dinov2_backbone.parameters():
                param.requires_grad = False
            load_model(teacher,state_dict)
        ...
        for sample in tqdm(train_dataloader):
            ...
            mask_gt = F.interpolate(mask_gt.float(), (95, 315)).long().to(device)
                            ...
                            for s in activated_slots:
                                ...
                                if num_features > 1:
                                    ...
                                    for ft in range(2, num_features + 1):
                                        if label_counts[ft] > 10 and mask_fg_t[i,j,0,:,:][labeled_mask == ft].mean() > 0.9:
        ...
        if not epoch % 10:
            ...
            image = image[0]
            image = F.interpolate(image, (95,315))
            masks = masks[0]
