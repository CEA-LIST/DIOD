...
from datasetPD_dinov2 import PDDataset
from datasetPDEval_dinov2 import PDDataset as PDDatasetEval
...
# Burn_in
parser.add_argument('--burn_in_ckpt', default='DIODPD_burn_in_400_dinov2.ckpt', type=str, help='ckpt from burn_in to load when starting distillation')
    ...
    for i in range(opt.batch_size):
        for j in range(5):
            ...
            for idx, s in enumerate(id_slot):
                ...
                fg_portion = torch.where(mask_gt[i,j,id_gt[idx],:,:]!=0)[0].size()[0]/(123*245)
...
def main():
    ...
    for epoch in range(opt.num_epochs):
        ...
        for sample in tqdm(train_dataloader):
            ...
            mask_gt = F.interpolate(mask_gt.float(), (123, 245)).long().to(device)
        ...
        if not epoch % 10:
            ...
            image = image[0]
            image = F.interpolate(image, (123,245))
            masks = masks[0]
