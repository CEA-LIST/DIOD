...
parser.add_argument('--ckpt_path', default='/home/users/skara/check_release/checkpoints/DIODPD_500.ckpt', type=str, help='pre-trained model path' )
...
def main():
    ...
    print('evaluating', opt.ckpt_path)
        ...
        if opt.is_bg_handled:  
            ...
            for i in range(40):        
                ...
                for fm in range(5): 
                    ...
                    gt_msk = F.one_hot(mask_gt[0,fm,:,:])
                   
                    pred_msk = F.one_hot(masks[0,fm,:,:,:].argmax(dim=0, keepdim=False))
                   
                    active_slots.append([pred_msk[:,:,s] for s in range(opt.num_slots-1) if pred_msk[:,:,s].max().item()>0])

                    scores.append([masks[0,fm,s,:,:][pred_msk[:,:,s] == 1].mean() for s in range(opt.num_slots-1) if pred_msk[:,:,s].max().item()>0])
        ...
        # evaluate other baselines w/o background modeling         
            ...
            for i in range(40):          
                ...
                for fm in range(5): 
                    ...
                    pred_msk = F.one_hot(masks[0,fm,:,:,:].argmax(dim=0, keepdim=False), num_classes=opt.num_slots)

                    active_slots.append([pred_msk[:,:,s] for s in range(opt.num_slots) if pred_msk[:,:,s].max().item()>0])
                    
                    scores.append([masks[0,fm,s,:,:][pred_msk[:,:,s] == 1].mean() for s in range(opt.num_slots) if pred_msk[:,:,s].max().item()>0])
