...
from datasetKITTIEval import KITTIDataset as KITTIDatasetEval
...
parser.add_argument('--exp_name', default='check_release_train_KITTI_100', type=str, help='experiment name, used for model saving/plotting ect' )
...
parser.add_argument('--num_epochs', default=101, type=int, help='number of workers for loading data')
...
# Burn_in
parser.add_argument('--burn_in_ckpt', default='DIODPD_500.ckpt', type=str, help='ckpt from burn_in to load when starting distillation')
    ...
    for i in range(opt.batch_size):
        for j in range(5):
            ...
            for idx, s in enumerate(id_slot):
                ...
                fg_portion = torch.where(mask_gt[i,j,id_gt[idx],:,:]!=0)[0].size()[0]/(92*312)
...
def main():
    ...
    for epoch in range(opt.num_epochs):
        ...
        for sample in tqdm(train_dataloader):
            ...
            mask_gt = F.interpolate(mask_gt.float(), (92, 312)).long().to(device)
                        ...
                        for j in range(5):
                            ...
                            for s in activated_slots:
                                ...
                                if num_features > 1:
                                    ...
                                    for ft in range(2, num_features + 1):  # starts from 1 as 0 is background
